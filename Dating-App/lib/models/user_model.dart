...
class UserModel extends Model {
  ...
  Future<User> getUserObject(String userId) async {
    ...
    final DocumentSnapshot<Map<String, dynamic>> userDoc = await UserModel()
        .getUser(userId);
  ...
  Future<void> verifyPhoneNumber({
    ...
    verificationComplete(fire_auth.AuthCredential authCredential) async {
      ...
      await _firebaseAuth.signInWithCredential(authCredential).then((
  ...
  Future<void> signUp({
    ...
    await _firestore
        ...
        .then((_) async {
          /// Get current user in database
          final DocumentSnapshot<Map<String, dynamic>> userDoc = await getUser(
            getFirebaseUser!.uid,
          );
  ...
  Future<void> updateProfileImage({
    ...
    if (path == 'profile') {
      ...
      await updateUserData(
        userId: user.userId,
        data: {USER_PROFILE_PHOTO: imageLink},
      );
    } else {
      /// Update gallery image
      await updateUserData(
        userId: user.userId,
        data: {'$USER_GALLERY.image_$index': imageLink},
      );
    }
  }
